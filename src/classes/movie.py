# Derived from tmdbv3api and modified for extra search functionality
import warnings
...
class Movie(TMDb):
    ...
    # For the purposes of UFS Bot, year is always included, adult always on
    def search(self, term, year, page=1, include_adult=True):
        """
        Search for movies.
        :param term:
        :param page:
        ...
        return self._get_obj(
            self._call(
                ...
                "query=" + quote(term) + "&page=" + str(page) + "&include_adult=" + str(include_adult) + "&year=" + str(year),
            )
        )
